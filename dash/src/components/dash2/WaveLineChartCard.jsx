...
export default function WaveLineChartCard() {
  const { isDark } = useTheme();

  const data = [
    { day: 'Sun', purple: 40, orange: 20 },
    { day: 'Mon', purple: 70, orange: 35 },
    { day: 'Tue', purple: 50, orange: 25 },
    { day: 'Wed', purple: 85, orange: 60 },
    { day: 'Thu', purple: 60, orange: 40 },
    { day: 'Fri', purple: 95, orange: 75 },
    { day: 'Sat', purple: 75, orange: 50 },
  ];
  ...
  return (
    <div className={`h-full min-h-[270px] p-6 rounded-3xl border transition-all duration-300 flex flex-col justify-between ${
      ...
      <div className="mb-1">
        <h3 className="text-sm font-bold tracking-tight">Commodo</h3>
        <p className="text-[10px] text-slate-400">Sed diam nonummy nibh euismod</p>
      </div>
      ...
      <div className="h-28 w-full mt-auto">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
            ...
            <YAxis hide />
            <Tooltip />
            <Area type="monotone" dataKey="purple" stroke="#6366f1" strokeWidth={2} fillOpacity={1} fill="url(#purpleGrad)" />
            <Area type="monotone" dataKey="orange" stroke="#fb923c" strokeWidth={2} fillOpacity={1} fill="url(#orangeGrad)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
