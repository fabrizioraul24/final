...
class AlmacenLotController extends Controller
{
    ...
    public function show(Request $request, Product $product): View
    {
        ...
        $warehouseId = $filters['warehouse_id'] ?? null;
        $resolvedWarehouseId = $this->resolvedWarehouseId($warehouseId);
        $lotScope = function ($query) use ($resolvedWarehouseId) {
            $query
        ...
        $productDetail = $this->decorateLotProducts(collect([$productDetail]))->first();

        $warehouses = $this->lotWarehouses();

        return view('dashboard.almacen-lotes-detalle', [
            'product' => $productDetail,
            'warehouse' => $warehouses->firstWhere('id', $resolvedWarehouseId) ?? $warehouses->first(),
            'filters' => [
                'warehouse_id' => $resolvedWarehouseId,
            ],
        ]);
