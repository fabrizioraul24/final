...
@section('content')
    @php
        ...
        $withEmail = $companies->filter(fn($company) => filled($company->email))->count();
    ...
    <style>
        .clients-report .summary { display: table; width: 100%; border-spacing: 0.45rem 0; table-layout: fixed; }
        .clients-report .summary-card { display: table-cell; width: 33.333%; min-height: 58px; padding: 0.55rem 0.7rem; vertical-align: top; }
        .clients-report .summary-card span { font-size: 0.98rem; }
        ...
        .client-table th,
        .client-table td { font-size: 0.68rem; padding: 0.42rem; vertical-align: top; }
        .client-table th:nth-child(1) { width: 28%; }
        .client-table th:nth-child(2) { width: 13%; }
        .client-table th:nth-child(3) { width: 17%; }
        .client-table th:nth-child(4) { width: 14%; }
        .client-table th:nth-child(5) { width: 28%; }
        .client-name { color: #1c1c2d; font-weight: 700; }
        .client-owner { display: block; margin-top: 0.12rem; color: #5f6a85; font-size: 0.58rem; }
        .type-pill { display: inline-block; padding: 0.18rem 0.46rem; border-radius: 999px; font-size: 0.62rem; font-weight: 700; }
    ...
    <div class="clients-report">
        <p class="report-note">
            Clientes filtrados por tipo {{ $filters['type_label'] ?? 'Todos' }}.
        </p>
        ...
        <table class="client-table">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="5">Sin clientes para el filtro seleccionado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
