...
export default function HorizontalBarsCard({ data }) {
  const { isDark } = useTheme();
  const categories = data?.categoryBreakdown || [];

  const monthsData = categories.length
    ? categories.map((item) => ({
        month: String(item.label || 'N/D').slice(0, 3).toUpperCase(),
        segments: [
          { w: `${Math.max(6, Number(item.share || 0))}%`, c: 'bg-indigo-600' },
          { w: `${Math.max(4, 100 - Number(item.share || 0))}%`, c: 'bg-indigo-300' },
        ],
      }))
    : [
  ...
  return (
    <div className={`p-6 rounded-3xl border transition-all duration-300 flex flex-col justify-between ${
      ...
      <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Categorias</h3>
