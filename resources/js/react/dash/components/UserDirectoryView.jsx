...
export default function UserDirectoryView() {
  ...
  const staffUsers = users.filter(u => u.role === 'Trainer' || u.role === 'Admin').length;

  // Filtered Users computation
  const filteredUsers = useMemo(() => {
    return users.filter(u => {
      // 1. Metric card filter
      if (activeFilterCard === 'ACTIVE' && u.status !== 'Active') return false;
      if (activeFilterCard === 'INACTIVE' && u.status !== 'Inactive') return false;
      if (activeFilterCard === 'STAFF' && u.role !== 'Trainer' && u.role !== 'Admin') return false;
  ...
  return (
    <div className="w-full max-w-[1500px] mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        >
          <div className="flex flex-col">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Entrenadores & Admins</span>
