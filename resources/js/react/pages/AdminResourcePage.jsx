...
function AgentReplenishmentPage({ data, flash, csrfToken }) {
    ...
    return (
        <>
            <FlashMessages flash={flash} />
            <div className="agent-hero"><div><h2 style={{ margin: 0, color: '#fff' }}>Reposicion inteligente</h2><p style={{ margin: '0.25rem 0 0', color: 'rgba(255,255,255,0.75)' }}>Evaluaciones del agente para anticipar faltantes y aprobar traspasos con control humano.</p>{data.error && <p style={{ margin: '0.55rem 0 0', color: '#fecdd3' }}>{data.error}</p>}</div><div className="agent-toolbar"><span className={`status-pill ${statusClass}`}><i className={data.agentOnline ? 'ri-checkbox-circle-line' : 'ri-close-circle-line'} />{data.agentOnline ? 'Agente en linea' : 'Agente sin conexion'}</span><span className="chip chip-muted">Ultima ejecucion: {data.lastRunAt}</span><form method="POST" action={data.routes.run}><input type="hidden" name="_token" value={csrfToken} /><button type="submit" className="pill-button"><i className="ri-play-circle-line" />Ejecutar analisis ahora</button></form></div></div>
