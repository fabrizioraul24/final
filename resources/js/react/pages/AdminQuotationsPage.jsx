...
export default function AdminQuotationsPage({ layout, data, flash, errors, old, csrfToken, logoutAction }) {
    ...
    const hasFilters = Boolean(data.filters?.search || data.filters?.sale_type);
    ...
    const metricCards = [
        ...
        { key: 'amount', label: 'Monto Total', value: `Bs ${Number(data.stats.total_amount || 0).toFixed(2)}`, hint: 'Cotizado', icon: 'ri-money-dollar-circle-line', tone: 'green' },
        { key: 'average', label: 'Promedio', value: `Bs ${Number(data.stats.average_amount || 0).toFixed(2)}`, hint: 'Por cotizacion', icon: 'ri-line-chart-line', tone: 'amber' },
        { key: 'items', label: 'Productos', value: data.stats.items_count || 0, hint: 'Items cotizados', icon: 'ri-box-3-line', tone: 'rose' },
    ];
            ...
            <div className="fit-users-page fit-quotations-page">
                ...
                <section className="fit-metric-grid fit-quotation-metric-grid">
                    {metricCards.map((card) => (
                        <a
                            key={card.key}
                            href={card.key === 'all' ? buildIndexUrl() : undefined}
                            className={`fit-metric-card ${card.tone}${card.key === 'all' ? ' active' : ' disabled'}`}
                        >
                            <span>
                ...
                <section className="fit-filter-card">
                    <form method="GET" action={data.routes.index} className="fit-filter-form fit-quotation-filter-form no-status">
                        ...
                        <div className="fit-table-scroll">
                            <table className="fit-users-table fit-quotations-table">
                                ...
                                <tbody>
                                        ...
                                        <tr key={quotation.id}>
                                            ...
                                            <td className="text-right">
                                                <div className="fit-row-actions">
                                                    <a href={quotation.show_url} className="fit-action-button success" title="Ver detalles">
                                    ...
                                    )) : <TableEmpty colSpan={6} text="No hay cotizaciones registradas." />}
                                </tbody>
                            </table>
                        </div>
                    </div>
