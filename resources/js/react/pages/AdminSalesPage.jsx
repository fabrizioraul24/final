    ...
    return (
        <DashboardShell sidebar={layout.sidebar} topbar={layout.topbar} csrfToken={csrfToken} logoutAction={logoutAction}>
            <div className="fit-users-page fit-sales-page">
                ...
                <Modal open={createSaleOpen} title="Registro Oficial de Ventas" onClose={() => setCreateSaleOpen(false)} wide contentClassName="fit-modal-content">
                ...
                <Modal open={!!detailSale} title={detailSale ? `Venta #${detailSale.id}` : 'Detalle de venta'} onClose={() => setDetailSale(null)} wide contentClassName="fit-modal-content">
                    {detailSale && (
                        <div className="fit-transfer-detail fit-sale-detail">
                            <div className="fit-transfer-summary fit-sale-summary">
                                <div><span>Cliente</span><strong>{detailSale.company?.name || detailSale.customer?.name || 'Sin cliente'}</strong></div>
                                <div><span>Tipo de venta</span><strong>{saleTypeLabels[detailSale.sale_type] || detailSale.sale_type}</strong></div>
                                <div><span>Estado</span><SaleStatus status={detailSale.status} /></div>
                                <div><span>Pago</span><strong>{detailSale.payment_label}</strong></div>
                                <div><span>Total</span><strong className="fit-sale-amount">Bs {Number(detailSale.total_amount).toFixed(2)}</strong></div>
                            </div>

                            <div className="fit-transfer-panel">
                                <h4>Entrega</h4>
                                <p>Almacen: {detailSale.warehouse?.name || '-'}</p>
                                <p>Ciudad: {detailSale.delivery_city || '-'}</p>
                                <p>Direccion: {detailSale.delivery_address || 'Sin direccion registrada.'}</p>
                                <p>Vendedor: {detailSale.seller?.name || '-'}</p>
                            </div>
